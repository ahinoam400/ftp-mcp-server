...
def _ftp_list_by_method(list_method: str, session_id: str, directory: str) -> str:
    """
...
def _ftp_store_methods_helper(session_id: str, is_unique_store: bool, local_filepath: str, 
    ...
    try:
        with open(local_filepath, 'rb') as f:
            if is_unique_store:
                # ftplib's stou() returns the server response string, not just the name.
                # We need to parse it to get the unique filename.
...
def ftp_store_file_unique(session_id: str, local_filepath: str) -> str:
    ...
    :param session_id: The unique ID of the active session.
    :param local_filepath: The path to the local file to upload.
    :return: The unique filename assigned by the server.
    """
...
def ftp_set_pasv(session_id: str, set_pasv: bool = True) -> str:
    ...
    :raises ToolError: If the command fails.
    """
    ftp = _check_session(session_id)
    try:
...
def ftp_send_port(session_id: str, host: str, port: int) -> str:
    ...
    try:
        ...
        raise ToolError(f"Failed to send PORT command: {e}")
...
def ftp_get_file_size(session_id: str, file_path: str) -> str:
    ...
    try:
        ...
        raise ToolError(f"Failed to get file size: {e}")
...
def ftp_void_command(session_id: str, command: str) -> str:
    ...
    :return: A confirmation message.
    :raises ToolError: If the command fails or returns data.
    """
    ftp = _check_session(session_id)
    try:
        ftp.voidcmd(command)
